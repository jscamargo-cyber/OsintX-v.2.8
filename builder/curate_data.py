...
def process():
    ...
    try:
        ...
        print("❌ Error: No se encuentra 'urls.txt' en la carpeta actual (/builder).")
        return
    ...
    for url in raw_urls:
        ...
        if match_key:
            ...
            cat, tags = determine_category_and_tags(url, name)
    ...
    # --- 3. CAMBIO: Ruta de salida a la carpeta PUBLIC ---
    js_content = f"window.tools = {json.dumps(tools, indent=2)};"
    
    # Esto sube un nivel (..) y entra en public/
    output_path = os.path.join('..', 'public', 'data.js') 
    ...
    try:
        ...
        print(f"❌ Error: No se encontró la carpeta {output_path}. Asegúrate de haber creado la carpeta 'public' al lado de 'builder'.")
